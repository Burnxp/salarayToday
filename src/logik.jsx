...
export function Rechner() {
  ...
  const feiertage = useFeiertage(
    inputValue ? new Date(inputValue).getFullYear() : new Date().getFullYear(),
  );
  ...
  const [show, setShow] = useState(() => {
    const saved = localStorage.getItem("lohnDaten");
    return saved ? JSON.parse(saved) : false;
  });

  //  Automatische Speicherung bei Änderung
  useEffect(() => {
    localStorage.setItem("lohnDaten", JSON.stringify(show));
  }, [show]); // Dieser Effekt läuft immer, wenn 'show' sich ändert
  ...
  const berechneLohn = () => {
    ...
    if (weekday(inputValue) === "Sonntag") {
      ...
      if (endTime < startTime) {
        ...
        console.log(
          "Sonntagszuschlag für Stunden vor Mitternacht:",
          sonntagsZuschlag,
        );
      }
    ...
    if (weekday(inputValue) === "Samstag") {
      ...
      if (endTime < startTime) {
        // Schicht geht über Mitternacht
        console.log(
          "Schicht geht über Mitternacht! Sonntagszuschlag berechnen.",
        );
    ...
    if (feiertage.includes(inputValue)) {
      if (endTime < startTime) {
        ...
        feiertagsZuschlag =
          feiertagsStd *
          parseFloat(stdLohn || 0) *
          (parseFloat(feiertagszuschlag || 0) / 100);
      }
      if (startTime < endTime) {
        feiertagsZuschlag =
          arbeitszeitStunden *
    ...
    let nachtZuschlag =
      nachtstunden *
      parseFloat(stdLohn || 0) *
      (parseFloat(nachtzuschlag || 0) / 100);
    gesamtLohn += nachtZuschlag;

    const feinPlanZuschlag = calculateFeinPlanZuschlag(
      feinPlanChecked,
      feinPlanHalfChecked,
      parseFloat(feinplanzuschlag || 0),
    );
  ...
  return (
    ...
    <>
      <div className="lohnrechner-layout ">
        {show && (
          <div className="flex-lohnDaten">
            <div className="aktLohndaten">
              ...
              Sonntagszuschlag: {localStorage.getItem(
                "Sonntagszuschlag",
              )} % <br />
              Feiertagszuschlag: {localStorage.getItem(
                "Feiertagszuschlag",
              )} % <br />
              Feinplanzuschlag: {localStorage.getItem(
                "Feinplanzuschlag",
              )} € <br />
            </div>
            ...
            <div className="pause-tageszuschlag">
              <div className="pause-container">
                <div className="pause-flex">
                  <label htmlFor="" className="pause-text">
                    Pause <span className="smal-text">(unbezahlt 30min)</span>
                  </label>
              ...
              <div
                className={`tageszuschlag ${parseFloat(feinplanzuschlag || 0) > 0 ? "visible" : "hidden"}`}
              >
        ...
        <div>
          <div>
            <div className="button-berechnung">
              <button className="berechnen-button" onClick={berechneLohn}>
                Berechnen
              </button>
            </div>
          </div>
        ...
        <div>
          <section className="seo-text text">
            <h2>Tageslohn im Schichtdienst einfach berechnen</h2>
            <p>
              Mit unserem{" "}
              <strong>
                kostenlosen und werbefreien Schichtlohnrechner /
                Gehaltsrechner{" "}
              </strong>
              berechnest du deinen <strong>Tageslohn im Schichtdienst</strong>{" "}
              schnell und zuverlässig. Trage deinen <strong>Stundenlohn</strong>{" "}
              und deine
              <strong> Arbeitszeit pro Tag</strong> ein und wähle aus, ob es
              sich um eine
              <strong> Nachtschicht</strong>, <strong> Feiertagsschicht</strong>{" "}
              oder reguläre Arbeitszeit handelt.
            </p>
            <p>
              Der Rechner berücksichtigt automatisch{" "}
              <strong>Nachtzuschläge</strong>,
              <strong> Feiertagszuschläge </strong> und weitere Schichtzulagen
              und zeigt dir deinen <strong>exakten Tagesverdienst</strong> an.
              So siehst du sofort, was du an einem Arbeitstag wirklich verdienst
              – <strong>transparent</strong>,<strong> anonym</strong> und{" "}
              <strong>ohne Anmeldung</strong>.
            </p>
          ...
          <div className="danke ">
            <h6>
              Freut mich, dass du die App nutzt! Wenn dir die Arbeit dahinter
              gefällt, kannst du mit einer kleinen Spende dazu beitragen, sie
